...
## Challenges
### 1. My Environment - 
I am working with a not-so-new laptop with Windows as my OS, so I needed to use a VM. I chose to use AWS EC2 and encountered little issues along the way.      First I Launched a t2.micro machine but as I tried to install minikube on it I found that it doesn't have 2 vCPUs so I needed to launche a new Machine with sufficiient resources. I launched the new machine and installed minikube. Finally,  as I tried to build the image it failed with a "no space left on device" error. to fix that I added more volume space to the EC2, expanded the partition and the filesystem. 
